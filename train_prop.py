...
from models.PerCNet import PerCNet
...
def train_pyg(
    ...
    wandb.init(project="PerCNet_MP_Gap")
    print(config)
    config = TrainingConfig(**config)

    wandb_config = wandb.config
    wandb_config.dataset = config.dataset
    wandb_config.target = config.target
    ...
    wandb.log({"test_mae": mean_absolute_error(np.array(targets), np.array(predictions))})
    print(
        "Test MAE:",
